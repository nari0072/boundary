...
def draw_atoms(context,width,height,cx,cy,mv,scale,lines1,lines2,opts={})
  ...
  #xz_atom
  pos_after.each{|pos|
    context.circle(mv+cx+adjust*pos[0],mv+adjust*pos[2], r)
    context.set_source_rgb(0, 0, 1)
    context.fill
    erasure_atom(lines1,lines2).each{|d|
      context.circle(mv+cx+adjust*d[0],mv+adjust*d[2], r)
      context.set_source_rgb(1, 0, 0)
      context.fill
    }
  }
  if opts[:same_size]
    (0..vector_max).each{|vector|
      ...
      context.move_to(mv+cx+adjust*pos_before[vector][0],mv+adjust*pos_before[vector][2])
      context.line_to(mv+cx+adjust*pos_after[vector][0],mv+adjust*pos_after[vector][2])
      context.set_line_width(1)
      context.stroke
    }
  end
  ...
  #yz_atom
  pos_after.each{|pos|
    context.circle(mv+adjust*pos[1],mv+cy+adjust*pos[2], r)
    context.set_source_rgb(0, 0, 1)
    context.fill
    erasure_atom(lines1,lines2).each{|d|
      context.circle(mv+adjust*d[1],mv+cy+adjust*d[2], r)
      context.set_source_rgb(1, 0, 0)
      context.fill
    }
  }
  if opts[:same_size]
    (0..vector_max).each{|vector|
      context.set_source_rgb(0, 0.8, 1)
      context.move_to(mv+adjust*pos_before[vector][1],mv+cy+adjust*pos_before[vector][2])
      context.line_to(mv+adjust*pos_after[vector][1],mv+cy+adjust*pos_after[vector][2])
      context.set_line_width(1)
      context.stroke
    }
  end
