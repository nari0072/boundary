...
def draw_axes
  ...
  [[0,1],[1,0]].each{|line|
    x,y=line[0],line[1]
    [[0,0],[$cx/2,0],[$cx,0],[$cx*4/3,0],[0,$cy],[0,$cy*3/2]].each{|c_x,c_y|
...
def draw_atoms
  draw_each_plane(0,1,$cx/2,$cy/2)
  draw_each_plane(0,2,$cx/2,$cy)
  draw_each_plane(1,2,$cx,$cy)
  draw_each_plane(1,2,$cx/5,$cy)
end
...
def draw_each_plane(ind_1,ind_2,c_x,c_y)
  ...
  [[$deleted_atoms,[1,0,0],rr*1.3],[$pos_after,[0,0,1],rr]].each{|atoms_color|
    ...
    atoms_color[0].each{|pos|
        $context.circle($mv+$cx/2+$adjust*pos[0], $mv+$cy*3/2+$adjust*pos[1],radius)
        $context.fill
        
        $context.circle($mv+$cx/2+$adjust*pos[0], $mv+$cy/2+$adjust*($pos_max[1]-pos[1]),radius)
        $context.fill
        
        $context.circle($mv+$cx/2+$adjust*pos[0], $mv+$cy+$adjust*(pos[2]),radius)
        $context.fill
        
        $context.circle($mv+$cx*4/3+$adjust*($pos_max[0]-pos[0]), $mv+$cy+$adjust*(pos[2]),radius)
        $context.fill
        
        $context.circle($mv+$cx+$adjust*pos[1], $mv+$cy+$adjust*pos[2],radius)
        $context.fill
        
        $context.circle($mv+$cx/5+$adjust*($pos_max[1]-pos[1]), $mv+$cy+$adjust*pos[2],radius)
        $context.fill
